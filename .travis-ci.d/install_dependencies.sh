...
# install dqm4hep-net
git clone https://github.com/dqm4hep/dqm4hep-online.git
cd dqm4hep-online
mkdir -p build && cd build
cmake -DINSTALL_DOC=OFF -Dxdrstream_DIR=$PWD/../../xdrstream -DCMAKE_MODULE_PATH=$PWD/../../dqm4hep/cmake -DDQMCore_DIR=$PWD/../../dqm4hep-core -DDQMNet_DIR=$PWD/../../dqm4hep-net ..

if [ $? -ne 0 ]; then
    echo "Failed to run dqm4hep-online cmake"
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "Failed to run dqm4hep-online make"
    exit 1
fi
cd ../..
